...
- Current branch, and the five most recent pending files in `buildlog/` (excluding `README.md`, ordered by entry date) plus the total pending count, or, if none are pending, the last entry in `BUILDLOG.md` on `main`.
