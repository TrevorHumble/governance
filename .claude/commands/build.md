...
**0b: Governance sync.** Run `powershell -File tools/governance-sync.ps1`. The merge decision defers to `standards/governance-sync.md`, not restated here. Branch on the outcome: when it reports the governance home, `not declared`, or `in sync`, continue to step 1. When it reports a sync PR opened or already open (and prints no `structure change:` line), run the contradiction review per `standards/governance-sync.md` and act per its outcomes, then, after a merge, re-cut this worktree from the updated default branch before continuing. Carry `.run_state/notes.md` across to the new worktree before continuing: it is gitignored, so the re-cut would otherwise drop every note this session has taken, per `agents/orchestrator.md` § "No agent files its own issue" rule 2. A missing file is the normal case, no notes taken yet, and needs no action; a copy that is attempted and fails is reported in the session rather than passed over silently.

When it prints a line starting with the literal prefix `structure change:` (the rule checker classified the run as structure, per `standards/governance-sync.md` § "What a sync is" and § "The standing-issue rule"), report the named child issue in the session and carry it into the end-of-run report, in either of its two shapes: `structure change: no sync PR` opens no PR at all, so continue the build on the governance already in the tree without re-cutting the worktree and without running the contradiction review, there is no PR to review, and a withheld sync never bricks a build, the same posture the sync-outage branch below already takes; `structure change: partial withhold` still opens a PR alongside the child issue (its withheld paths are named in the issue, never in the PR), so also run the contradiction review per `standards/governance-sync.md` on that PR and act per its outcomes, then, after a merge, re-cut this worktree from the updated default branch before continuing, same as an ordinary sync PR.
