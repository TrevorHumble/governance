...
`tests/classify-dep-pr.test.js` is repo-local: this rule's `paths:` frontmatter names it so a change to the tier logic re-triggers this rule in this repo, but a child that lacks `tests/` simply never matches that path and the rule fires on nothing.
