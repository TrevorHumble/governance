...
- [ ] Changed files match the `Touches` field in the issue's dependency map, or a widening recorded there per `standards/adversarial-review-protocol.md` § "Finding disposition", "Recording a widening" paragraph (that paragraph is the single owner of the record's shape and where it lands). Verify a recorded widening against its stated reason, do not just check the file list against the issue as originally filed; an unannounced, unrecorded file is still a finding. This check does not see every widening: one taken to fix a round-1 minor finding ships with no independent re-review at all, since a minor is fixed inline under the one-round stop rule, so on that path the `Touches`-line record is an audit trail this reviewer reads, not a gate it enforces.
