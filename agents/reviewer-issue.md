...
**Input:** the absolute path to the issue file under review. Read that file, `standards/issue-standards.md`, `standards/pipeline/templates/hand-off-format.md`, `standards/reviewer-conduct.md`, `standards/pipeline/templates/spawn-skeleton.md`, and `definition-of-done.md` (repo root, for the trapped-vs-wanting test applied below). For an issue on a declared Pre-review surface (as defined at the top of this file's Protocol note), also read `standards/pipeline/edge/unchanged-artifact-exemption.md`. For an issue carrying a `Night-pass:` marker, also read `standards/pipeline/edge/night-pass.md` and `CLAUDE.md` (repo root, for the governing-artifact surface list that row checks against). Beyond that, read only files in this repository needed to test a claim the issue makes: including confirming that a file path named in the implementation plan or `Touches` list resolves to a real file, or is genuinely new.
...
- [ ] If the issue carries a `Night-pass:` marker, check it against `standards/pipeline/edge/night-pass.md`: the marker is well-formed per § "Marker", and the issue meets every condition in § "What it covers". Any failure is a blocking FAIL.
