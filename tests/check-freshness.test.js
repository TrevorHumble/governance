...
// Regression guard for /realign step ordering. `.claude/commands/realign.md`
...
maybeDescribe('realign ordering: overlap must be read before fast-forward', () => {
  ...
  // sibling has merged a src/app.js rewrite to origin/main, so `clone`'s local
...
// single-homed fact, one owner. (This repo has no config.js or other
// consumer that could duplicate it, so only the single-assignment case
// applies here.)
